...
#
  ...
  context "loading example vcloud-edge configurations" do
    examples_dir = File.join(
      File.dirname(__FILE__),
      '..', '..', 'examples'
    )
    Dir["#{examples_dir}/vcloud-edge/*.yaml"].each do |input_file|
      it "should load example config #{File.basename(input_file)}" do
        loader = Vcloud::ConfigLoader.new
        actual_config = loader.load_config(input_file, ::Vcloud::Schema::EDGE_GATEWAY_SERVICES)
